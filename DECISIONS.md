...
**Onboarding is self-serve — and so is the rest of the lifecycle.** The home page uploads a `.json`/`.csv` export (drag-drop or browse); validation at upload is shape-only — is it parseable at all — because mapping is the interactive part the mapper owns. Uploading a file whose company already exists deliberately offers *replace* rather than erroring: that is the drift-maintenance flow — the saved config is kept and the next preview reports exactly what changed in the new export. Companies can also be removed without touching the filesystem: ✕ on the home card deletes the raw file, the saved mapping, and the company's cached LLM extractions (matched by *exact* note text via the cache's content hash — a substring of another company's note must not match), while "delete saved mapping" in the Config JSON tab's danger zone resets to a fresh draft with the raw file kept.
